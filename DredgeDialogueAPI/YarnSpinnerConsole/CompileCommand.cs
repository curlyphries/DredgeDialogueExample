    ...
    public static class CompileCommand
    {
        public static void CompileFiles(FileInfo[] inputs, DirectoryInfo outputDirectory, string outputName, string outputStringTableName, string outputMetadataTableName, bool stdout)
        {
            try { 
                ...
                {
                    WinchCore.Log.Warn(diagnostic);
                }

                if (compiledResults.Diagnostics.Any(d => d.Severity == Diagnostic.DiagnosticSeverity.Error))
                {
                    WinchCore.Log.Error($"Not compiling files because errors were encountered.");
                ...
                {
                    try { compiledResults.Program.WriteTo(codedStream); } catch (Exception ex) { WinchCore.Log.Info(ex); }
                }

                WinchCore.Log.Info($"Wrote {programOutputPath}");
                ...
                WinchCore.Log.Info($"Wrote {stringTableOutputPath}");
                ...
                using (var writer = new StreamWriter(stringMetadatOutputPath))
                {
                    ...
                    WinchCore.Log.Info($"Wrote {stringMetadatOutputPath}");
                }
            } catch (Exception ex) { WinchCore.Log.Info(ex); }
        }
        ...
        public static FileInfo[] GetYarnFiles(FileInfo[] inputs)
        {
            ...
            /*if (anyFileIsProject)
            {
                if (inputs.Length > 1)
                {
                    WinchCore.Log.Fatal($"When compiling a Yarn Project file, you must specify only a single file path.");
                }
